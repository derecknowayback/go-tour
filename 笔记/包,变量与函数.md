...
func main() {
	fmt.Println(math.Pi) // math.pi是错误的 
}
```

小写开头，那么就不能包外使用；大写开头，才可以在包外使用；

**做了个内外部隔离，类似java private 和 c static，只不过不需要关键字修饰，用大小写来区分；**
...
var i, j int = 1, 2 // 不省略类型

func main() {
	var c, python, java = true, false, "no!" // 省略类型
...
func main() {
	...
	fmt.Println(needFloat(Small)) // Small 是 Float
	fmt.Println(needFloat(Big)) // Big 是 float
}
```
